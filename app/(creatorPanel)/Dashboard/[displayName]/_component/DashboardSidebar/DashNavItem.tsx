...
const DashNavItem = ({ label, icon:Icon, href, isActive}: NavItemProps) => {
    const {collapsed} = useCreatorSidebar((state) => state)
    const activeStyle = isActive ? 'bg-accent' : ' goldText';
  return (

    <Button
    asChild
    variant='ghost'
    className={cn('w-full h-12', collapsed ? 'justify-center' : "justify-start", activeStyle)}
    // key={key}
    >
        <Link href={href}>
